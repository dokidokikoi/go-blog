...
func (c *Controller) List(ctx *gin.Context) {
	...
	if input.Keyword != "" {
		...
		items, total, err = c.srv.Link().ListByWhereNode(ctx, example, node, listOption)
	} else {
		items, total, err = c.srv.Link().List(ctx, example, listOption)
	}
	if err != nil {
		zaplog.L().Error("获取友链列表失败", zap.Error(err))
